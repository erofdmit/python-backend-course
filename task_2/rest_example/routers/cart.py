...
from ..errors import CartNotFoundException, ItemNotFoundException
...
async def create_new_cart(
    response: Response, conn: Connection = Depends(get_db_connection)
):
...
async def get_carts(
    ...
    min_price: Optional[float] = Query(
        None, ge=0.0, description="Минимальная цена корзины"
    ),
    max_price: Optional[float] = Query(
        None, ge=0.0, description="Максимальная цена корзины"
    ),
    min_quantity: Optional[int] = Query(
        None, ge=0, description="Минимальное общее количество товаров"
    ),
    max_quantity: Optional[int] = Query(
        None, ge=0, description="Максимальное общее количество товаров"
    ),
    conn: Connection = Depends(get_db_connection),
):
    """Получение списка корзин с фильтрацией по цене и количеству товаров."""
    carts = await get_cart_list(
        conn, offset, limit, min_price, max_price, min_quantity, max_quantity
    )
    return carts
...
async def add_item_to_cart_endpoint(
    ...
    quantity: Optional[int] = Query(
        1, gt=0, description="Количество добавляемого товара"
    ),
    conn: Connection = Depends(get_db_connection),
):
